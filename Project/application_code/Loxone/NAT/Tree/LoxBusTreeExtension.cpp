...
LoxBusTreeExtension::LoxBusTreeExtension(LoxCANBaseDriver &driver, uint32_t serial, eAliveReason_t alive)
  : LoxNATExtension(driver, (serial & 0xFFFFFF) | (eDeviceType_t_TreeBaseExtension << 24), eDeviceType_t_TreeBaseExtension, 0, 11021110, 5, sizeof(config), &config, alive), treeDevicesLeftCount(0), treeDevicesRightCount(0), leftDriver(this, eTreeBranch_leftBranch), rightDriver(this, eTreeBranch_rightBranch) {
}
