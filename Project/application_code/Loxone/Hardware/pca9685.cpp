...
static const uint8_t PINOUT[] = {3, 2, 1, 0, 7, 11, 10, 9, 8, 4, 5, 6};
...
bool pca9685_init(pca9685_handle_t *handle)
{
	...
	// Set mode registers to default values (Auto-Increment, Sleep, push pull).
...
bool pca9685_set_channel_duty_cycle_float(pca9685_handle_t *handle, unsigned channel, float duty_cycle, bool logarithmic)
{
...
bool pca9685_set_channel_duty_cycle(pca9685_handle_t *handle, unsigned channel, uint16_t duty_cycle, bool logarithmic)
{
	...
	if (duty_cycle == 0) {
		...
		if (logarithmic) {
			required_on_time = CIEL_8_12[(unsigned)roundf(duty_cycle>>4)];
		} else {
			required_on_time = (unsigned)roundf(duty_cycle);
		}
